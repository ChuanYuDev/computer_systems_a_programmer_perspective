...
int main(int argc, char **argv) 
{
    ...
    while (1)
    {
        ...
        /* If simply input enter, the cmdline is "\n\0" */
...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv))
    {
        ...
        Sigprocmask(SIG_BLOCK, &mask_all, NULL); /* Block all the signals */

        state = bg ? BG : FG;
        addjob(jobs, pid, state, cmdline);
        jid = pid2jid(pid);

        /* TO DO: How to handle add too many jobs? Refer to tshref */
...
int builtin_cmd(char **argv) 
{
    ...
    if (!strcmp(cmd, "jobs"))
    {
        ...
        listjobs(jobs);
        Sigprocmask(SIG_SETMASK, &prev_all, NULL); /* Unblock all the signals */
...
void waitfg(pid_t pid)
{
    ...
    Sigprocmask(SIG_BLOCK, &mask_all, &prev_all); /* Block all the signals to access jobs */
...
/* Examine fork function return value */
pid_t Fork(void)
{
    pid_t pid;
...
int Sigprocmask(int how, const sigset_t *set, sigset_t *old_set)
{
    ...
    if ((rc = sigprocmask(how, set, old_set)) < 0)
